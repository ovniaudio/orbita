...
- **ORBIT reports its latency, and the host compensates it exactly.** It reports **424 samples
  ...
  knob. Before, the plugin reported **0** while actually delaying between 25 and 445 samples
  depending on that knob, so the DAW could not align it. The dry path is delayed by the same
...
- **The plug-ins are now named `OVNI ORBIT.vst3` and `OVNI ORBIT.component`** (they were
  ...
  - The installer **retires the old bundle for you**, from both `/Library/Audio/Plug-Ins` and
    your own `~/Library/Audio/Plug-Ins` — but **only if it identifies itself as
